...
- **안정성(beta/volatility)이 낮은 종목군의 초과성과는 시장 국면에 좌우된다** —
  ...
  팩터 패턴이다.
- **가격 기반 지표(beta/volatility/return/rsi)만으로는 개별 종목의 상대적
  성과를 예측할 수 없다** — 지도학습 모델 3종 모두 정확도·AUC가 0.49~0.50
  (동전 던지기 수준). 연도별 walk-forward 검증(2021~2026, 6개 fold)에서도
  모든 해에 걸쳐 일관되게 예측력이 없음을 재확인했다.
...
- **팩터 레벨로 보면 Risk(beta·volatility·부채비율)가 압도적으로 가장 중요한
  ...
  합친 것보다 크다. 비지도(클러스터링)와 지도(분류)라는 독립적인 두 방법론이
  "이 시장을 설명하는 가장 강한 축은 안정성"이라는 같은 결론으로 수렴한다.
