...
TRUSTED_RENAMES = {
    ...
    # 2026-09-03 추가 — 사명변경이 아니라 '상장 시장 이동'이라 성격이 다르다.
    # FRC(First Republic Bank)는 2023-05-01 FDIC 관리 편입 후 NYSE에서 폐지되고
    # 핑크시트에서 FRCB로 계속 거래됐다. 같은 법인이고 야후가 파산 전 NYSE 이력을
    # FRCB 심볼로 그대로 서빙한다(2021년 고점 $219.90이 실제 FRC 고점과 일치).
    # SIVB와 동일한 구조라 같은 기준으로 처리한다 — 파산 전 실거래 구간은 유효한
    # 데이터이고, 좀비 구간(종가 $0.01 이하, 2023-11-28~)은 멤버십 필터가
    # 편출일(2023-05-04, 실제 파산일과 일치)에서 이미 차단한다. 제외하면 오히려
    # "구성종목이 4개월 만에 -97%" 라는 실제 사건이 표본에서 사라져 왜곡된다.
    "FRC": "FRCB",
}
