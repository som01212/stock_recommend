...
def predict_all(fitted: dict, X: pd.DataFrame) -> pd.DataFrame:
    ...
    For portfolio selection a hard yes/no per stock is exactly what's
    needed, so this stays 0/1 -- use predict_proba_all() instead when
    scoring ranking quality (e.g. AUC).
    """
...
def predict_proba_all(fitted: dict, X: pd.DataFrame) -> pd.DataFrame:
    """Each model's predicted probability of class 1 (outperform).

    roc_auc_score needs a score that ranks examples, not a hard 0/1 call --
    feeding it predict()'s binary output collapses the ROC curve to the
    single point implied by each model's default 0.5 cutoff, which is just
    balanced accuracy wearing an AUC label. Use this for AUC; use
    predict_all() for actual stock selection.
    """
    models = fitted["models"]
    X_scaled = fitted["scaler"].transform(X)
    ...
    return pd.DataFrame(
        {
            ...
            "logistic": models["logistic"].predict_proba(X_scaled)[:, 1],
